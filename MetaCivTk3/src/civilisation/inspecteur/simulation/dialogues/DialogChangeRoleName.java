...
public class DialogChangeRoleName extends JDialog implements ActionListener, PropertyChangeListener{
	
	JTextField nom;
    JOptionPane optionPane;
    GroupToolBar source;
    
	public DialogChangeRoleName(Frame f , boolean modal , GroupToolBar source){
		super(f,modal);

		this.source = source;
		nom = new JTextField(20);
		nom.setText("role_name");
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if (isVisible() && (optionPane.getValue().equals("OK") || optionPane.getValue().equals("Cancel"))){
			if (optionPane.getValue().equals("OK")){
				source.changeRoleName(nom.getText());
			}		
	        setVisible(false);
		}
