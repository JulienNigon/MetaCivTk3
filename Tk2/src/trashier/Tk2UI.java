 ...
 * @version 1.4 - 4/2010
 */
public class Tk2UI extends JFrame implements TabListener{
	...
	private void initContent(){
		this.setTitle(" * * * T U R T L E K I T   2 . 2 * * *  ");
	 ...
	 */
	public void createSimulationWindow(String group, SimulationRunner launcher, Tk2SimuUIManager uiManager){
		Tk2Tab newTab = createTab(uiManager.myGUI);
		newTab.l = launcher;
		tabbedPanel.addTab(newTab);
		tabbedPanel.setSelectedTab(newTab);
		((SimulationUI)(newTab.getContentComponent())).setTheme(dockingThemes[activeTheme]);
	}
	...
	private void fillViewerMenu(Collection<XMLAttributes> att, final SimulationRunner l) {
		...
		for (Iterator<XMLAttributes> iterator = att.iterator(); iterator.hasNext();) {
			...
			viewerMenu.add(createMenuItem(xmlAttributes.getString("ViewerClass"), 
					new ActionListener() {
				public void actionPerformed(ActionEvent e) {
...
//			((SimulationUI)(tabbedPanel.getTabAt(i).getContentComponent())).setTheme(dockingThemes[theme]);
		((SimulationUI)(tabbedPanel.getSelectedTab().getContentComponent())).setTheme(dockingThemes[theme]);
	}
