...
public class WorldViewer extends Viewer
{
	...
	{
		super.setup();
		GraphismesAmeliores = this.getBooleanParam("GraphismesAmeliores" , true); //n'est plus utilisé pour l'instant
	...
	public void paintPatch(Graphics g, Patch p,int x,int y,int cellS){
			...
			if (p.isMarkPresent("Champ"))
			{
				...
				p.dropMark("Champ", mark); // On repose la mark, car elle est retirée à l'appel du get…
			}
	...
	public void paintTurtle(Graphics g,Turtle t,int x,int y,int cellSize)
    {
		...
		// Les dessins sur le carré de couleur
